<div class="module" id="drill-char-vth">
	<h1>Choose the character matching the sound</h1>
	<p class="drill-instruction">
		This drill plays an audio clip of a random japanese syllable/sound.  You must choose the corresponding hiragana character from the possibilities in the table below. Once the correct character is chosen, the next sound will play. The settings to the right can be used to use Google's alternate audio voice or to limit the range of characters being tested.
	</p>
	<div class="button-box">
		<button class="play-button">Play Again</button>
	</div>
	<div class="char-choice-holder">
	</div>
	<div class="button-box inline">
		<button class="cheat-button drill-button">Cheat</button>
	</div>
	<div class="button-box inline">
		<button class="next-button">Skip</button>
	</div>
</div>
